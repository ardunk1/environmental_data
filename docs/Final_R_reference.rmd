...
### require()
The require() function behaves similarly to the library() function; however, it has a benefit. When the require() function is called, it will first check to see if a package is already loaded. If it is loaded, require() will not re-load the package, which can save time and processing power. 

Use require() in an example:
```{r}
...
ginkgo <- read.csv(here("data", "ginkgo_data_2021.csv")) #create a data object from the csv that is saved in the data folder
...
The vector contents can be shown by calling the vector name or using the print() function
```{r}
...
### matrix()
The function matrix() creates a matrix of rows and columns (i.e., 2-dimensional data) of the same type of data object (i.e., all numeric, all character, etc.). Matrices are organized by rows first, and columns second when discussing dimensions.
...
### data.frame()
The data.frame() function builds data frames that are similar to matrices in that they are organized by rows and columns  (i.e., 2-dimensional), but differ from matrices in that each column (variable) can contain a different type of data (i.e., character, numberic, boolean).
...
head(Adelie) #check that we selected all rows of data associated with Adelie species
...
mean(ginkgo$max_width, na.rm = TRUE) #na.rm = TRUE states that NA will be removed prior to calculating the mean.
...
The plot() function produces a scatterplot of an x and y value. This simple plot can then be stylized using a series of arguments. The syntax is plot(x vector, y vector.)
...
Below are a series of options to control the break points along the x axis. Each of the examples uses "breaks =" with various assigned values. There are challenges and benefits to each option.
...
### boxplot() (Box Plots)
The boxplot() function allows you to visualize continuous response data with a categorical predictor.

Here is a simple boxplot that graphically shows the summary information for one continuous response variable:
```{r}
...
Conditional boxplot:
```{r}
boxplot(petiole_length ~ seeds_present, data = ginkgo, xlab = "Seeds Present")

```

### par()
The par() function controls graphical parameters. It can be used to adjust margins in the figure and to arrange multiple graphs into one figure by using par(mfrow = )

Here's the example: 
```{r}
par(mfrow = c(2,2))
hist(penguins$bill_length_mm, main = "Bill Length", xlab = "Bill Length (mm)", col = "blue")
hist(penguins$bill_depth_mm, main = "Bill Depth", xlab = "Bill Depth (mm)", col = "green")
hist(penguins$flipper_length_mm, main = "Flipper Length", xlab = "Bill Depth (mm)", col = "purple")
hist(penguins$body_mass_g, main = "Body Mass", xlab = "Body Mass (g)", col = "orange")
...
The dnorm() function is the probability density function for normal continuous distributions.It returns the probability of a given value along a normal distribution. In other words, it gives the height of the probability distribution for a given x value.

The arguments include:

*   x - the value(s) you want to know the probability of
*   mean - the distribution mean
*    sd - the distribution standard deviation
*    log - defaults to FALSE, can be overridden to TRUE if you have a logarithmic distribution

```{r}
dnorm(2, mean = 5, sd = 2) #For a normal distribution with a mean of 5 and a standard deviation of 2, there is a 6.5% chance of observing the value 2

```

### pnorm()

The pnorm() function is the cumulative density function for the normal continuous distribution. It returns the probability of observing any value less than or equal to a given value. In other words, it gives the total area under the curve from x=0 to x=given argument. 

The arguments are the same as for dnorm() with the addition of: 

*   lower.tail = TRUE - which is the default and returns the cumulative probability of less than the given number, or
*   lower.tail = FALSE - which returns the inverse of TRUE (1-lower.tail = TRUE)

```{r}

pnorm(2, mean = 5, sd = 2) #6.7% chance of observing a value of 2 or less (remember dbinom provided only the chance of observing a value of 2)
pnorm(2, mean = 5, sd = 2, lower.tail = FALSE) #93.3% chance of observing a value greater than 2 (the inverse of above)

```

### qnorm()

The qnorm() function is the quantile density function for the normal continuous distribution. It returns the value from the distribution at a given quantile.

The arguments are the same as the dnorm except for the entered value, which represents a quantile as a decimal value.

```{r}
qnorm(0.90, mean = 5, sd = 2) #At the 90% percentile, we would observe a value of 7.5
...
### dbinom()
The dbinom() function is the probability mass function for the binomial discrete distribution with two outcomes. This function provides the probability density distribution for any given number of "successes" in the provided distribution. 

The arguments are:

*   x - the number of successes you're calculating
*   size - number of trials
*   prob - probability of success (must be the same for every trial)
*   log = FALSE - default, can be over-ridden to TRUE

```{r}
dbinom(3, 10, 0.5) #11.7% chance of getting 3 successes in 10 trials with a probability of success of 50%

```

### pbinom()
The pbinom() function is the cumulative mass function for the binomial discrete distribution. This function provides the probability of observing a given number or fewer successes. 

The arguments are the same as the dbinom function, except:

*   q - probability, instead of x. Similar to the pnorm, 
*   lower.tail and log can be set to TRUE or FALSE.


```{r}
pbinom(3, 10, 0.5) #17% chance of getting 3 or fewer success in 10 trials with a probability of success of 50%. 

```

### qbinom()

The qbinom() function is the quantile function for the binomial discrete distribution. This function provides the number of successes that would be observed within a given percentile of the distribution.

The arguements are the same as the dbinom() except for the entered value, which represents a quantile as a decimal value.

```{r}
qbinom(.17, 10, 0.5) # At the 17th percentile, with 10 trials and a success rate of 50%, you would observe 3 successes.
