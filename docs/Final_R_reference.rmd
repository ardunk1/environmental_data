...
### par()
par() controls graphical parameters. It can be used to adjust margins in the figure and to arrange multiple graphs into one figure by using par(mfrow = )
...
dnorm() is the probability density function for normal continuous distributions.It returns the probability of a given value along a normal distribution. In other words, it gives the height of the probability distribution for a given x value.

The arguments include:
x - the value(s) you want to know the probability of
mean - the distribution mean
sd - the distribution standard deviation
log - defaults to FALSE, can be overridden to TRUE

```{r}
dnorm(2, mean = 5, sd = 2)
#For a normal distribution with a mean of 5 and a standard deviation of 2, there is a 6.4% chance of observing the value 2

```

### pnorm()

pnorm() is the cumulative density function for the normal continuous distribution. It returns the probability of observing any value less than or equal to a given value.In other words, it gives the total area under the curve from x=0 to x=given argument. 

The arguments are the same as for bnorm() with the addition of: 
lower.tail = TRUE - which is the default and returns the cumulative probability of less than the given number and
lower.tail = FALSE - which return the inverse of TRUE (1-lower.tail = TRUE)
```{r}

pnorm(2, mean = 5, sd = 2) #0.0668
pnorm(2, mean = 5, sd = 2, lower.tail = FALSE) #0.9333

```

### qnorm()

qnorm() is the quantile density function for the normal continuous distribution. It returns the value from the distribution at a given quantile.

The arguments are the same as the bnorm except for the entered value, which represents a probabilty.

```{r}
qnorm(0.90, mean = 5, sd = 2) #value of 7.5
...
### dbinom()
dbinom() is the probability mass function for the binomial discrete distribution with two outcomes. This function provides the probability density distribution for any given number of "successes" in th provided distribution. 
...
```{r}
dbinom(3, 10, 0.5) #11% chance of getting 3 successes in 10 trials with a probability of success of 50%

```

### pbinom()
pbinom() is the cumulative mass function for the binomial discrete distribution. This function provides the probability of observing a given number or fewer successes. 

The arguments are the same as the dbinom function, except:
q - probability, instead of x. Similar to the pnorm, the lower.tail and log can be set to TRUE or FALSE.
...
qbinom() is the quantile function for the binomial discrete distribution. This function provides the number of successes that would be observed within a given quantile of the distribution.
